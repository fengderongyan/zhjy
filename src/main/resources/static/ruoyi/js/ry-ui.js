    ...
    $.extend({
    	...
    	table: {
            ...
            initEvent: function(data) {
            	...
            	$.btTable.on('click', '.img-xs', function() {
    			    var src = $(this).attr('src');
    			    var target = $(this).data('target');
    			    var height = $(this).data('height');
    			    var width = $(this).data('width');
			...
			imageCommonView: function (value, height, width, target) {
				if ($.common.isEmpty(width)) {
                	width = 'auto';
                }
                if ($.common.isEmpty(height)) {
				...
				var _target = $.common.isEmpty(target) ? 'self' : target;
				if ($.common.isNotEmpty(value)) {
					return $.common.sprintf("<img class='img-xs' data-height='%s' data-width='%s' data-target='%s' src='%s'/>", width, height, _target, value);
				} else {
					return $.common.nullToStr(value);
				}
			},
    		...
    		importTemplate: function(fileName) {
    			if($.common.isEmpty(fileName)){
    				$.get($.table._option.importTemplateUrl, function(result) {
    				 ...
    				 window.location.href = ctx + "common/templateDownload?fileName=" + encodeURI(fileName);
    			}
            ...
            //通用导入
            importCommonExcel: function(formId) {
            	var currentId = $.common.isEmpty(formId) ? 'importCommonTpl' : formId;
            	console.log("路径：：" + $.table._option.importCommonUrl);
            	layer.open({
            		type: 1,
            		area: ['400px', '230px'],
            		fix: false,
            		...
            		btn1: function(index, layero){
            			...
            			formData.append("file", $('#file')[0].files[0]);
            			$.ajax({
            				url: $.table._option.importCommonUrl,
            				data: formData,
            				cache: false,
            				contentType: false,
            				processData: false,
